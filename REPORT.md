...
개인 실습 기록. 모두의연구소 Agt1 "[실습 프로젝트] 라우팅 에이전트 만들기"의 구조와 보고 항목을 이 서비스에 적용해 봄 (과제 제출물 아님).
...
- 실험 9회 (유지 5, 되돌림 2, 보류 1, 적용 전 기각 1). 전체 기록 [runs/LAB.md](runs/LAB.md)
...
- 고치지 않기로 한 것
  ...
  - 실습용 안내 문서를 새로 쓰면 "내가 쓴 문서로 내가 평가"하는 구조가 됨 → 게시 문서만 근거로 쓰고, 이 공백은 넘기기 문항으로 활용
...
| 9 | 검증: LLM이 답변 문장마다 조회 조항 근거 판정 | 0.976 / 1.000 | 88.1 / 92.9 | 85.7 / 90.5 | 76.2 / 85.7 | 보류 (기본 꺼 둠) |
...
- **처리 과정을 강의 구조 그대로 네 칸으로**: ① 분류(라우트·확신도·이유) → ② 판정 → ③ 근거 조회 → ④ 검증
...
- **의미 수준 검증 다듬기** (실험 9에서 구현, 보류)
  - 오프라인 시험에서 금지 내용 넣은 답안 48/48, E-008 누출, 채점기도 못 잡은 E-034 조항 왜곡을 잡음
  - 파이프라인에 붙이면 점수 개선 없음. 오탐(모범 답안 약 6~10%)이 재생성 중 불필요한 넘김으로 이어짐
  - 다음: 재생성 피드백에서 넘기기 권유 빼기, 판정 두 번 일치할 때만 걸기
